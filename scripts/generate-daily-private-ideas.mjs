...
function geminiTransportSchema(schema) {
  ...
  const property = (node) => {
    ...
    if (type === "object") {
      ...
      if (Object.keys(properties).length === 0) return { type };
      const required = (resolved.required || []).filter((name) => properties[name]);
    ...
    const transportArray = { type, items: property(resolved.items) || { type: "object" } };
    for (const key of ["minItems", "maxItems"]) {
      if (Number.isInteger(resolved[key])) transportArray[key] = resolved[key];
    }
