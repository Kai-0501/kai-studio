...
test("Gemini structured output receives the compatible response schema", () => {
  ...
  try {
    const request = providerRequest("Return one object", { type: "object", minProperties: 1, properties: { name: { type: "string" }, ideas: { type: "array", items: { type: "object" } } } }, "gemini");
    const payload = JSON.parse(request.init.body);
    assert.equal(payload.generationConfig.responseMimeType, "application/json");
    assert.deepEqual(payload.generationConfig.responseSchema, { type: "object", properties: { name: { type: "string" }, ideas: { type: "array", items: { type: "object" } } }, required: ["name", "ideas"] });
  } finally {
