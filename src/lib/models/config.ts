...
const defaultModels: ModelDefinition[] = [
  ...
  { id: "local.gemma4-31b-security", displayName: "Gemma 4 31B", provider: "ollama", providerModel: process.env.KAI_MODEL_SECURITY ?? "gemma4:31b-mlx", capabilities: ["chat", "security-review", "repository-review", "structured-output", "reasoning", "vision"], contextWindow: 131072, supportsTools: false, supportsStructuredOutput: true, supportsVision: true, supportsReasoning: true, defaultTemperature: 0, defaultContextAllocation: 98304, enabled: true },
  ...
  { id: "local.image-generator", displayName: "Local Image Generator", provider: "ollama", providerModel: process.env.KAI_IMAGE_MODEL ?? "x/z-image-turbo:latest", capabilities: ["image-generation"], contextWindow: 8192, supportsTools: false, supportsStructuredOutput: false, supportsVision: false, supportsReasoning: false, defaultTemperature: 0, defaultContextAllocation: 8192, enabled: true },
...
const defaultRoutes: Record<ModelRole, RoleRoute> = {
  ...
  "vision.reviewer": { primary: "local.gemma4-31b-security", requiredCapabilities: ["vision", "structured-output"], localOnly: true },
  "image.planner": { primary: "local.gemma4-31b-security", requiredCapabilities: ["chat", "structured-output"], localOnly: true },
  "image.generator": { primary: "local.image-generator", requiredCapabilities: ["image-generation"], localOnly: true },
