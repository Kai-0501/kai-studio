...
export default function GreenfieldPage() {
  return <Suspense fallback={<AppShell><section className="mx-auto w-full max-w-4xl p-8 text-slate-400">Opening Greenfield Workspace…</section></AppShell>}><GreenfieldContent /></Suspense>;
}

function GreenfieldContent() {
  const handoff = useSearchParams();
  const [templates, setTemplates] = useState<Template[]>([]);
  const [root, setRoot] = useState("");
  const [templateId, setTemplateId] = useState("nextjs");
  const [name, setName] = useState(() => handoff.get("projectName") ?? "");
  const [message, setMessage] = useState("");
  const plannedTask = handoff.get("source") === "kai-agent" ? handoff.get("task") : null;
